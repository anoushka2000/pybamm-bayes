...
data = pd.read_csv(
    "/Users/anoushkabhutani/PycharmProjects/battery-model-parameterization/battery_model_parameterization/Python/old notebooks/schimpe_data_cell1.csv")
...
log_prior_D_sol = pints.GaussianLogPrior(-17, 1)
D_c_sol = Variable(name="D_c_sol", value=-17.6 + 3.42, prior=log_prior_D_sol)
variables[limit_option] = [D_c_sol, ]

limit_option = "electron-migration limited"  # to run
log_prior_U_inner = pints.UniformLogPrior(0.1, 0.8)
...
limit_option = "ec reaction limited"  # running
...
# limit_option = "solvent-diffusion limited" {"D_c_sol": D_c_sol.value}
# limit_option = "interstitial-diffusion limited" {"D_c_li": D_c_li.value}
...
estimation_problem = ParameterEstimation(
    ...
    initial_soc=1,
    transform_type="None",
    project_tag=f"Q_{limit_option}",
)
estimation_problem.plot_data()
estimation_problem.plot_priors()

chains = estimation_problem.run(
    n_iteration=4000, n_chains=5, n_workers=3
)

estimation_problem.plot_results_summary(forward_evaluations=700)
