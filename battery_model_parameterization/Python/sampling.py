...
def run_parameter_estimation(
    ...
    Parameters
    __________
    parameter_estimation_problem: ParameterEstimationProblem
        Problem to run MCMC for.
    burnin: int
        Initial iterations discarded from each chain.
    n_iteration: int
    ...
    xs = [
        x * parameter_estimation_problem.initial_values
        for x in np.random.normal(1, 0.2, n_chains)
    ]
    ...
    mcmc.set_log_to_screen(True)
    mcmc.set_chain_filename(
        os.path.join(parameter_estimation_problem.logs_dir_path, "chain.csv")
    )
    mcmc.set_chain_storage(store_in_memory=True)
    mcmc.set_log_pdf_filename(
        os.path.join(parameter_estimation_problem.logs_dir_path, "log_pdf.csv")
    )
    ...
    #  find residual at optimal value
    y_hat = parameter_estimation_problem.simulate(
        theta_optimal, times=parameter_estimation_problem.times
    )
    error_at_optimal = np.sum(abs(y_hat - parameter_estimation_problem.data)) / len(
        parameter_estimation_problem.data
    )

    pd.DataFrame(
        {
            "residuals": parameter_estimation_problem.residuals,
        }
    ).to_csv(os.path.join(parameter_estimation_problem.logs_dir_path, "residuals.csv"))

    with open(
        os.path.join(parameter_estimation_problem.logs_dir_path, "metadata.json"),
        "r",
    ) as outfile:
        metadata = json.load(outfile)
