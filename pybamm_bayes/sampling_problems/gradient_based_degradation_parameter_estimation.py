...
class DegradationParameterEstimation(BaseSamplingProblem):
    ...
    def _get_state_of_health(self, inputs):
        ...
        if "Q_Li" in inputs.keys():
            ...
            Q_Li = self.parameter_values.evaluate(self.symbolic_params.Q_Li_particles_init)
        ...
        try:
            ...
            try:
                ...
                self.soh_parameter_values.process_model(model)
                solver = pybamm.CasadiAlgebraicSolver(extra_options={"enable_fd": True,},)
                sol = solver.solve(
                            model, [0], 
                            inputs = inputs,
    ...
    def simulateS1(self, theta: np.ndarray, times: np.ndarray):
        """
        Wrapper around _simulate method to handle timeouts.
        """
        def handler(signum, frame):
