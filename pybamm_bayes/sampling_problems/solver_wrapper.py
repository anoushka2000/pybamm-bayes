...
class IDAKLUJax:
    ...
    def _jaxify_solve(self, t, invar, *inputs_values):
        ...
        # inputs_values = tuple(float(i) for i in self.jax_inputs.values())
        if self.jax_inputs is not None:
            ...
            for key, value in zip(self.jax_inputs.keys(), inputs_values):
                d[key] = value # self.jax_inputs[key]
        # Solver
    ...
    def _jax_solve(
        ...
        *inputs,
        ...
        logger.debug(f"  t: {type(t)}, {t}")
        # print(f"_jax_solve  inputs: {type(inputs)}, {inputs}")
        # Returns a jax array
    ...
    def _jax_vjp_impl(
        ...
        *primals: np.ndarray,
        ...
        if t.ndim == 0 or (t.ndim == 1 and t.shape[0] == 1):
            ...
            y_dot = jnp.zeros_like(t)
            # print(f"_jax_vjp_impl  inputs: {type(inputs)}, {inputs}")
            js = self._jaxify_solve(t, invar, *inputs)
            if js.ndim == 0:
                js = jnp.array([js])
    ...
    def _jaxify(
        ...
        *,
        ...
        def f(t, inputs=None):
            ...
            print(type(flatargs))
            print(type(flatargs))
            print(f"flatargs {flatargs}")
            n_inputs = len(inputs.keys())
            flatargs[-n_inputs:] = inputs.values()
            print(f"modified flatargs {flatargs}")
